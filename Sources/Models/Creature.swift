class Creature {
    ...
    func move(in direction: Direction, mapBounds: (minX: Int, maxX: Int, minY: Int, maxY: Int)) -> String {
        ...
        case .north:
            ...
            position.y += 1
        case .south:
            if position.y - 1 < mapBounds.minY { return "\(name) can't move south." }
            position.y -= 1
        case .east:
            if position.x + 1 > mapBounds.maxX { return "\(name) can't move east." }
            position.x += 1
        case .west:
            if position.x - 1 < mapBounds.minX { return "\(name) can't move west." }
            position.x -= 1
        }
